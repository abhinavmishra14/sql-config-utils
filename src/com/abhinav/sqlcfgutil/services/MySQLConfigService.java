...
public class MySQLConfigService {
	...
	private String url;
      ...
      */
	public MySQLConfigService(final String cfgPath, final String dbName, final String userName, final String password,
	...
	public void executeStatements() throws ClassNotFoundException, SQLException, IllegalArgumentException {
		...
		if (dbName != null && !ConfigConstants.EMPTY.equals(dbName)) {
			System.out.println("[MySQLConfigProcessor:] Processing request for database name:- "
					+ dbName);
			try{
				...
				conn = DBConnectionUtil.getConnection(ConfigConstants.MYSQLDRIVER,url,
						userName,password);
				queries = dbCfgMdl.getStatements().get(dbName);
				for (final Iterator<String> iterator = queries.iterator(); iterator.hasNext();) {
					final String query = iterator.next().trim();
					System.out.println("[MySQLConfigProcessor:] Executing query: "+query);
					try{
			...
			for (final Iterator<Entry<String, DBConfigModel>> iterator = configMap.entrySet().iterator();
				...
				System.out.println("[MySQLConfigProcessor:] Processing request for database name:- "
						+ dbName);
				try{
					conn = DBConnectionUtil.getConnection(ConfigConstants.MYSQLDRIVER,url.concat(dbName),
							userName,password);
